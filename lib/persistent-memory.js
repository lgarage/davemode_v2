...
try {
  ...
  if (typeof VectorMemory !== 'function') {
    throw new Error("VectorMemory is not a constructor");
  }
} catch (error) {
  ...
  VectorMemory = class MockVectorMemory {
    constructor() {
      this.initialized = false;
    }
    async initialize() { 
      console.log("MockVectorMemory initialized (disabled)");
      return false; 
    }
    ...
    async addVectors(collectionName, embeddings, metadatas, ids) { 
      console.log("Mock: Adding " + embeddings.length + " vectors to " + collectionName);
      return; 
    }
    async queryVectors(collectionName, queryEmbeddings, nResults = 5) { 
      console.log("Mock: Querying vectors in " + collectionName);
...
class PersistentMemorySystem {
  ...
  async initialize() {
    ...
    try {
      // Initialize structured memory if it has an initialize method
      if (typeof this.structuredMemory.initialize === 'function') {
        await this.structuredMemory.initialize();
      } else {
        console.log("StructuredMemory does not have initialize method, skipping...");
      }
  ...
  async getRelevantContext(query, projectName) {
    ...
    try {
      ...
      try {
        similarInteractions = await this.vectorMemory.findSimilarInteractions(query, 10);
      } catch (error) {
